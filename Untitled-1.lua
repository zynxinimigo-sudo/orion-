...
function Library:CreateWindow(Settings)
    ...
    local GlobalSearchContainer = Create("Frame", {
        Parent = TopBar,
    ...
    local SearchIconGlobal = Create("ImageLabel", {
        Parent = GlobalSearchContainer,
        Size = UDim2.new(0, 16, 0, 16),
        Position = UDim2.new(0, 8, 0.5, -8),
        BackgroundTransparency = 1,
    ...
    local GlobalSearchBox = Create("TextBox", {
        Parent = GlobalSearchContainer,
        Size = UDim2.new(1, -32, 1, 0),
    ...
    AddCorner(GlobalSearchBox, 6)
    local GlobalSearchStroke = Create("UIStroke", { Parent = GlobalSearchBox, Color = SelectedTheme.TextDark, Transparency = 0.7, Thickness = 1 })
    table.insert(Library.ThemeObjects.TextDark, GlobalSearchStroke)
    GlobalSearchBox.Focused:Connect(function()
        TS:Create(GlobalSearchStroke, TweenInfo.new(0.2), { Color = SelectedTheme.ElementAccent, Transparency = 0 }):Play()
    end)
    GlobalSearchBox.FocusLost:Connect(function()
        TS:Create(GlobalSearchStroke, TweenInfo.new(0.2), { Color = SelectedTheme.TextDark, Transparency = 0.7 }):Play()
    end)
    ...
    local function FilterCurrentTabElements()
        if not ActiveTab or not ActiveTab.Page then return end
        local search = GlobalSearchBox.Text:lower()
        local page = ActiveTab.Page
        local function recursiveFilter(parent)
            for _, child in ipairs(parent:GetChildren()) do
                if child:IsA("TextButton") or child:IsA("Frame") then
                    local nameLabel = child:FindFirstChildWhichIsA("TextLabel")
                    if nameLabel and nameLabel.Text then
                        local matches = (search == "" or nameLabel.Text:lower():find(search))
        ...
        recursiveFilter(page)
    end

    GlobalSearchBox:GetPropertyChangedSignal("Text"):Connect(FilterCurrentTabElements)
    ...
    function Funcs:CreateTab(TabName, Two_Column, ImageID)
        ...
        local function GetElements(TargetParent)
            ...
            function Elements:CreateDropdown(Cfg)
                ...
                local SearchStroke = Create("UIStroke", { Parent = SearchBox, Color = SelectedTheme.TextDark, Transparency = 0.7, Thickness = 1 })
                table.insert(Library.ThemeObjects.TextDark, SearchStroke)
                SearchBox.Focused:Connect(function()
                    TS:Create(SearchStroke, TweenInfo.new(0.2), { Color = SelectedTheme.ElementAccent, Transparency = 0 }):Play()
                end)
                SearchBox.FocusLost:Connect(function()
                    TS:Create(SearchStroke, TweenInfo.new(0.2), { Color = SelectedTheme.TextDark, Transparency = 0.7 }):Play()
                end)
                local ItemsContainer = Create("Frame", {
                    Parent = Container,
                ...
                local ItemsLayout = Create("UIListLayout", { Parent = ItemsContainer, Padding = UDim.new(0, 4) })
                local function Update(val)
                    Selected = val
                ...
                local function RefreshList(newOptions)
                    ...
                    for _, item in pairs(Options) do
                        if search == "" or tostring(item):lower():find(search) then
                    ...
                    if not found and Selected ~= nil then
                        Selected = nil
                        Title.Text = Cfg.Name or "Dropdown"
                        if Cfg.Callback then Cfg.Callback(nil) end
                    ...
                    ItemsContainer.CanvasSize = UDim2.new(0, 0, 0, #filtered * 32 + 10)
                    Container.CanvasSize = UDim2.new(0, 0, 0, ItemsLayout.AbsoluteContentSize.Y + 40)
                end
                RefreshList(Options)
                ...
                Drop.MouseButton1Click:Connect(function()
                    ...
                    local contentHeight = ItemsLayout.AbsoluteContentSize.Y + 40
                    local H = Expanded and math.min(contentHeight + 44, 180) or 34
            ...
            function Elements:CreatePlayerDropdown(Cfg)
                ...
                local ThumbCache = {}
                local function GetPlayerThumbnail(uid)
                ...
                local function RefreshList()
                    ...
                    for _, plr in ipairs(Plrs:GetPlayers()) do
                        if plr ~= LP then
                            local nameMatch = plr.DisplayName:lower():find(search) or plr.Name:lower():find(search)
                            if search == "" or nameMatch then
                    ...
                    for uid, info in pairs(Disconnected) do
                        local nameMatch = info.DisplayName:lower():find(search) or info.Name:lower():find(search)
                        if search == "" or nameMatch then
                            local isSelected = Selected[uid] ~= nil
                            local Btn = Create("TextButton", {
                                Parent = ItemsContainer,
                    ...
                    local totalH = rows * 48 + 10
                    Container.CanvasSize = UDim2.new(0, 0, 0, totalH)
                    ItemsContainer.Size = UDim2.new(1, 0, 0, totalH)
                    if Expanded then
                        local H = math.min(totalH + 44, 220)
                ...
                Plrs.PlayerRemoving:Connect(function(plr)
                    local uid = plr.UserId
                    if Selected[uid] then
                        Disconnected[uid] = { Name = plr.Name, DisplayName = plr.DisplayName }
                        Selected[uid] = { Name = plr.Name, DisplayName = plr.DisplayName, Disconnected = true }
                    end
                    SafeRefresh()
                    UpdateTitle()
                end)
                ...
                Drop.MouseButton1Click:Connect(function()
                    ...
                    local H = Expanded and math.min(rows * 48 + 44, 220) or 34
                    TS:Create(Drop, TweenInfo.new(0.4, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), { Size = UDim2.new(1, 0, 0, H) }):Play()
                    TS:Create(Arrow, TweenInfo.new(0.4, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), { Rotation = Expanded and 180 or 0 }):Play()
                    TS:Create(Container, TweenInfo.new(0.4, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), { Size = UDim2.new(1, 0, 1, -34) }):Play()
                end)
